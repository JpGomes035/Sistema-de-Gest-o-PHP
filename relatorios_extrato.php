...
<body>
    <h1>Relatório de Extrato - Pedidos Pagos e Recebidos</h1>
    ...
    if (isset($_GET['dataInicio']) && isset($_GET['dataFim'])) {
        ...
        $sqlPedidos = "SELECT codigo_pedido, nome_cliente AS nome, valor_total, data, fm_pag, banco_receb, 'Recebido' AS tipo, pago
                       FROM pedidos 
                       WHERE data BETWEEN '$dataInicio' AND '$dataFim' 
                       AND deletado = 'N' AND pago = 'S'";

        // Query para buscar os pedidos de compra (pagos)
        $sqlCompras = "SELECT codigo_pedido, nome_fornecedor AS nome, valor_total, data, fm_pag, banco_receb, 'Pago' AS tipo, pago 
                       FROM pedido_compra 
                       WHERE data BETWEEN '$dataInicio' AND '$dataFim' 
                       AND deletado = 'N' AND pago = 'S'";
    ...
    if (isset($_GET['exportar']) && $_GET['exportar'] === 'excel') {
        ...
        while ($array = mysqli_fetch_array($resultado, MYSQLI_ASSOC)) {
            ...
            echo '<td>' . ($array['pago'] == 'S' ? 'Sim' : 'Não') . '</td>';
            echo '</tr>';
